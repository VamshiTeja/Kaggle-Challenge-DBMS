...
# @Last Modified time: 2018-04-15 20:00:59
import sys
import os
import numpy as np
import pandas as pd
...
def get_ratings():
    ...
    try:
        ...
        avg_table_dict = dict(zip(avg_table[:,0],avg_table[:,1]))
        item_avg_dict = dict(zip(item_avg_table[:,0],item_avg_table[:,1]))

        print(len(avg_table_dict),len(item_avg_dict))

        for (i,row) in enumerate(test_data):
            print(i)
            score = float(avg_table_dict[row[0]])+float(item_avg_dict[row[1]])
            print score
            test_rats.append(score)
